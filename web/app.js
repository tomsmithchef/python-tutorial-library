...
async function initCommunityBoard() {
  ...
  if (!isBoardConfigured()) {
    boardMessage(
      "#community-status",
      "Supabase is not connected yet. Add your project URL and anon key to web/community-config.js, then run the SQL setup file in Supabase."
    );
...
function updateAuthUi() {
  ...
  if (isSignedIn) {
    ...
    signedInLabel.textContent = `Signed in as ${name}`;
...
async function createPost() {
  ...
  const { error } = await boardState.client.from("board_posts").insert(payload);
  if (error) {
    boardMessage("#post-message", error.message, true);
    return;
  }
...
async function loadRelatedData(posts) {
  ...
  if (postIds.length) {
    const { data: comments } = await boardState.client
      .from("board_comments")
      .select("*")
      .in("post_id", postIds)
  ...
  if (userIds.size) {
    const { data: profiles } = await boardState.client
      .from("profiles")
